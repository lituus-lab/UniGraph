...
"""Build unigraph._core, a Cython extension over the UniGraph C ABI.
...
PKG_DIR = os.path.join(HERE, "unigraph")
VENDOR_DIR = os.path.join(HERE, "_nimsrc")
NIMBLE_FILE = "UniGraph.nimble"
...
if sys.platform == "win32":
    LIB_NAME, BUNDLED = "UniGraph.lib", False
    LINK_ARGS, NIMBLE_TASK = [], "clibMsvc"
elif sys.platform == "darwin":
    LIB_NAME, BUNDLED = "libUniGraph.dylib", True
    LINK_ARGS, NIMBLE_TASK = ["-Wl,-rpath,@loader_path"], "clib"
else:
    LIB_NAME, BUNDLED = "libUniGraph.so", True
...
def nim_project_dir():
    """Where UniGraph.nimble lives: the real repo root in a normal checkout,
    or the vendored copy when building from an extracted sdist (which has no
    parent repo, just this project standalone)."""
...
def ensure_lib_built():
    ...
    if os.path.exists(built):
        return built
    try:
        subprocess.check_call(["nimble", "install", "-y", "--depsOnly"], cwd=proj)
        subprocess.check_call(["nimble", NIMBLE_TASK], cwd=proj)
    except FileNotFoundError as e:
        raise SystemExit(
            "setup.py: `nimble` not found on PATH. Building unigraph from "
...
# The sdist ships the pre-transpiled unigraph/_core.c, not the .pyx (Cython
...
# needing no Cython.
pyx = os.path.join("unigraph", "_core.pyx")
ext = Extension(
    "unigraph._core",
    sources=[pyx if os.path.exists(os.path.join(HERE, pyx)) else os.path.join("unigraph", "_core.c")],
...
setup(
    ...
    package_data={"unigraph": [LIB_NAME] if BUNDLED and not IS_SDIST else []},
    exclude_package_data={"unigraph": ["_core.c"]},
    zip_safe=False,
)
