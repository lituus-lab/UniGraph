...
suite "one version, six copies":
  let manifest = valueOf("UniGraph.nimble", "version", "\"", "\"")
  ...
  test "the C header agrees, macros and string alike":
    ...
    check valueOf("include/UniGraph.h", "UNIGRAPH_VERSION_MAJOR", " ",
        "") == parts[0]
    check valueOf("include/UniGraph.h", "UNIGRAPH_VERSION_MINOR", " ",
        "") == parts[1]
    check valueOf("include/UniGraph.h", "UNIGRAPH_VERSION_PATCH", " ",
        "") == parts[2]
    check valueOf("include/UniGraph.h", "define UNIGRAPH_VERSION ", "\"",
        "\"") == manifest

  test "the C ABI reports it":
    # Read from the source rather than called: this suite links no C library.
    check valueOf("src/UniGraph/c_api.nim", "UniGraphVersionC", "\"",
        "\"") == manifest
  ...
  test "the Python test expects it":
    check valueOf("py/tests/test_graph.py", "unigraph.version()", "\"",
        "\"") == manifest
